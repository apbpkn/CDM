...
#include <PressureDependMultiYield04.h>
#include <LadeDuncanMultiYield.h>
#include <FluidSolidPorousMaterial.h>
#include <PressureIndependMultiYieldDamage.h>
...
/////////////////////////////////////////////////////////////////
	...
	// Pressure Independend Multi-yield Damage Model, by Zhijian Qiu
	else if (strcmp(argv[1], "PressureIndependMultiYieldDamage") == 0) {
		const int numParam = 11;
		const int totParam = 16;
		int tag;  double param[totParam];

		param[11] = 0.0;     // friction
		param[12] = 0.0;     // residual friction
		param[13] = 100.;    // refPress  
		param[14] = 0.0;     // PressdepenCoe  
		param[15] = 20;     // NYS 

		char * arg[] = { "nd", "rho", "refShearModul", "refBulkModul", "cohesi", "OctShearStra1"
			"residualCohesion","OctShearStra2","SofteningNum1", "SofteningNum2", "StiffnessReduction"
		...
		if (argc < (3 + numParam)) {
			...
			opserr << arg[10] << "? " << arg[11] << "? " << arg[12] << "? " << arg[13] << "? " << endln;
			return TCL_ERROR;
		}

		if (Tcl_GetInt(interp, argv[2], &tag) != TCL_OK) {
			opserr << "WARNING invalid PressureIndependMultiYieldDamage tag" << endln;
			return TCL_ERROR;
		}
		...
		for (int i = 3; (i<argc && i<19); i++)
			if (Tcl_GetDouble(interp, argv[i], &param[i - 3]) != TCL_OK) {
				...
				opserr << "nDMaterial PressureIndependMultiYieldDamage: " << tag << endln;
				return TCL_ERROR;
			}
		...
		PressureIndependMultiYieldDamage * temp =
			new PressureIndependMultiYieldDamage(tag, param[0], param[1], param[2],
				param[3], param[4], param[5], param[6],
				param[7], param[8], param[9], param[10], param[11], param[12], param[13], param[14], param[15], gredu);
		theMaterial = temp;
	...
	// Pressure Independend Multi-yield Damage Model, by Zhijian Qiu
	else if (strcmp(argv[1], "PressureIndependMultiYieldCyclic") == 0) {
		const int numParam = 12;
		const int totParam = 17;
		int tag;  double param[totParam];

		param[12] = 0.0;     // friction
		param[13] = 0.0;     // residual friction
		param[14] = 100.;    // refPress  
		param[15] = 0.0;     // PressdepenCoe  
		param[16] = 20;     // NYS 

		char * arg[] = { "nd", "rho", "refShearModul", "refBulkModul", "cohesi", "OctShearStrain"
			"residualCohesion","SofteningNum1", "SofteningNum2", "SofteningNum3", "SofteningNum4", "Cvalue"
			"frictionAng (=0)","residualFriction (=0)","refPress (=100)","pressDependCoe (=0.0)", "numberOfYieldSurf (=20)" };
		if (argc < (3 + numParam)) {
			opserr << "WARNING insufficient arguments\n";
			printCommand(argc, argv);
			opserr << "Want: nDMaterial PressureIndependMultiYieldCyclic tag? " << arg[0];
			opserr << "? " << "\n";
			opserr << arg[1] << "? " << arg[2] << "? " << arg[3] << "? " << "\n";
			opserr << arg[4] << "? " << arg[5] << "? " << arg[6] << "? " << "\n";
			opserr << arg[7] << "? " << arg[8] << "? " << arg[9] << "? " << endln;
			opserr << arg[10] << "? " << arg[11] << "? " << arg[12] << "? " << arg[13] << "? " << arg[14] << "? " << arg[15] << "? " << endln;
			return TCL_ERROR;
		}

		if (Tcl_GetInt(interp, argv[2], &tag) != TCL_OK) {
			opserr << "WARNING invalid PressureIndependMultiYieldCyclic tag" << endln;
			return TCL_ERROR;
		}

		for (int i = 3; (i<argc && i<21); i++)
			if (Tcl_GetDouble(interp, argv[i], &param[i - 3]) != TCL_OK) {
				opserr << "WARNING invalid " << arg[i - 3] << "\n";
				opserr << "nDMaterial PressureIndependMultiYieldCyclic: " << tag << endln;
				return TCL_ERROR;
			}
  ...
  // Pressure Dependend Multi-yield 03 (based on PDMY02). JL
		...
		param[numParam + 11] = 0.0;
		param[numParam + 12] = 1.0;
		param[numParam + 13] = 0.0;
		param[numParam + 14] = 0.0;
		param[numParam + 15] = 1.0;
		...
		char * arg[] = { "nd", "rho", "refShearModul",
			...
			"e (=0.6)", "volLimit1 (=0.9)", "volLimit2 (=0.02)",
			"volLimit3 (=0.7)", "Atmospheric pressure (=101)", "cohesi (=.1)",
			"mType(=0)","B1(=1.0)", "B2(=0.0)", "B3(=0.0)", "B4(=1.0)",
			"Hv (=0)", "Pv (=1.)" };
		if (argc < (3 + numParam)) {
			opserr << "WARNING insufficient arguments\n";
			printCommand(argc, argv);
	...
	// nDMaterial PressureDependMultiYield04  $tag  $nd  $rho  $refShearModul  $refBulkModul  
	...
	// PressureDependMultiYield04 (based on PressureDependMultiYield02). 
	else if (strcmp(argv[1], "PressureDependMultiYield04") == 0) {
		...
		if (argc < (3 + numParam)) { // 3 refers to "nDMaterial PressureDependMultiYield04  $tag"
			opserr << "WARNING insufficient arguments\n";
			printCommand(argc, argv);
			opserr << "Want: nDMaterial PressureDependMultiYield04 tag? " << arg[0];
			opserr << "? " << "\n";
			opserr << arg[1] << "? " << arg[2] << "? " << arg[3] << "? " << "\n";
			opserr << arg[4] << "? " << arg[5] << "? " << arg[6] << "? " << "\n";
			opserr << arg[7] << "? " << arg[8] << "? " << arg[9] << "? " << "\n";
		...
		if (Tcl_GetInt(interp, argv[2], &tag) != TCL_OK) {
			opserr << "WARNING invalid PressureDependMultiYield04 tag" << endln;
			return TCL_ERROR;
		}
		...
		for (int i = 3; (i<argc && i<in); i++)
			if (Tcl_GetDouble(interp, argv[i], &param[i - 3]) != TCL_OK) {
				opserr << "WARNING invalid " << arg[i - 3] << "\n";
				opserr << "nDMaterial PressureDependMultiYield04: " << tag << endln;
				return TCL_ERROR;
			}
		...
		PressureDependMultiYield04 * temp =
			new PressureDependMultiYield04(tag, param[0], param[1], param[2],
				param[3], param[4], param[5],
				param[6], param[7], param[8],
				param[9], param[10], param[11],
				param[12], param[13], param[14],
	...
	// LadeDuncanMultiYield Zhijian Qiu
		...
		if (argc < (3 + numParam)) {
			...
			opserr << arg[19] << "? " << arg[20] << "? " << arg[21] << "? " << "\n";
			opserr << arg[22] << "? " << arg[23] << "? " << arg[24] << "? " << "\n";
			opserr << arg[25] << "? " << arg[26] << "? " << "? " << endln;
			return TCL_ERROR;
		}

		if (Tcl_GetInt(interp, argv[2], &tag) != TCL_OK) {
			opserr << "WARNING invalid LadeDuncanMultiYield tag" << endln;
			return TCL_ERROR;
		}
		...
		for (int i = 3; (i<argc && i<in); i++)
			if (Tcl_GetDouble(interp, argv[i], &param[i - 3]) != TCL_OK) {
				opserr << "WARNING invalid " << arg[i - 3] << "\n";
				opserr << "nDMaterial LadeDuncanMultiYield: " << tag << endln;
				return TCL_ERROR;
			}
